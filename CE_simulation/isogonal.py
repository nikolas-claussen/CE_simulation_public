...
# %% auto 0
__all__ = ['polygon_area', 'polygon_perimeter', 'get_vertex_energy', 'get_shape_tensor', 'get_triangle_shape_tensor',
           'get_shape_energy', 'rotate_about_center']
...
# %% ../03_real_shape_optimization.ipynb 11
...
def get_triangle_shape_tensor(poly: NDArray[Shape["N, 2"], Float], metric=True):
    
    center = anp.mean(poly, axis=0)
    triangles = anp.stack([anp.stack(poly.shape[0]*[center]), poly, anp.roll(poly, 1, axis=0)], axis=1)
    ...
    edges = poly - anp.roll(poly, 1, axis=0)
    if metric: # don't normalize
        return 2 * anp.mean(anp.einsum("ei,ej->ije", edges, edges)*orientations, axis=-1) # 
    lengths = anp.linalg.norm(edges, axis=-1) 
    tensor = 2*anp.mean(anp.einsum("ei,ej->ije", edges, edges)*orientations/lengths, axis=-1) # *orientations
    return tensor
    ...
    #else:
    #    triangle_lengths = anp.linalg.norm(triangle_edges, axis=-1)
    #    triangle_units = (triangle_edges.transpose((2,0,1)) / triangle_lengths).transpose((1,2,0))
    #    tensor = anp.mean(anp.einsum("vei, vej->ijv", triangle_edges, triangle_units)*orientations, axis=-1)
    
    #tensor = anp.mean(anp.einsum("vei, vej->ijv", triangle_edges, triangle_edges), axis=-1)
    
    #return 4/poly.shape[0]*tensor
    
    #edges = poly - anp.roll(poly, 1, axis=0)
    #tensor =  2 * anp.mean(anp.einsum("ei,ej->ije", edges, edges), axis=-1)
    
    #return tensor + .05*polygon_area(poly[::-1]) * anp.eye(2)
...
@patch
def get_shape_tensors(self: HalfEdgeMesh, metric=False):
    ...
    for v in self.vertices.values():
        ...
        if not (None in neighbors):
            ...
            result_dict[v._vid] = get_triangle_shape_tensor(polygon, metric=metric)
        else: # leave at default value
            result_dict[v._vid] = np.eye(2)
    return result_dict

@patch
def set_rest_shapes(self: HalfEdgeMesh, metric=False):
    """Set rest shapes to current shapes"""
    shape_dict = self.get_shape_tensors(metric=metric)
    for v in self.vertices.values():
        v.rest_shape = shape_dict[v._vid]

# %% ../03_real_shape_optimization.ipynb 32
@patch
def get_energies(self: HalfEdgeMesh, mod_shear=0, mod_bulk=1, A0=1, P0=1, energy="v"):
