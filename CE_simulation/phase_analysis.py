...
# %% auto 0
__all__ = ['get_width_height', 'get_delta_centerline', 'get_anisos_S', 'get_weighted_unweighted_q', 'make_segments3d',
           'colorline3d']
...
def get_weighted_unweighted_q(mesh):
    """Get q, q*n, n order parameters"""
    tensors = np.stack([fc.get_stress_tensor() for fc in mesh.faces.values()])
    tensors = (tensors.T / np.linalg.eigvalsh(tensors).sum(axis=1)).T # normalize
    tensors = tensors - np.einsum('i,xy->ixy', np.trace(tensors, axis1=1, axis2=2)/2, np.eye(2))

    ns = np.linalg.eigh(tensors).eigenvectors[...,-1]
    phis = np.arctan2(*ns.T)
    qs = np.linalg.eigh(tensors).eigenvalues[...,-1]

    unweighted = np.abs(np.mean(np.exp(1j*2*phis)))
    weighted = np.abs(np.mean(qs*np.exp(1j*2*phis)))
    q = np.mean(qs)
    return [q, weighted, unweighted]
