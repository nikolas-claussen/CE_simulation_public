...
def run_phase_diagm_sim(shear_active, shear_passive, n_steps, random_seed=None, run_id=0):
    """
    ...
    """
    mod_shear = shear_active
    rel_shear_modulus = 1-(shear_passive/shear_active)
    energy_args['mod_shear'] = mod_shear

    # define directiory for saving stuff, save "metadata"
    save_dir = f"{base_dir}/{run_id}/shearActive_{shear_active}_shearPassive_{shear_passive}_rep_{random_seed}"
    try:
...
shear_actives = [0.01, 0.1, 0.5, 1, 2, 4, 8] #  4, 0.001
shear_passives = [1,]
reps = [1, 2, 3]

to_do = list(itertools.product(shear_actives, shear_passives, reps))

n_steps = 250 #125
run_id = "germ_band_new_post_T1_shear_new_passive_long"

###############################################
# run simulation
###############################################

Parallel(n_jobs=21, prefer=None)(delayed(run_phase_diagm_sim)( 
    shear_active, shear_passive,
    n_steps=n_steps, random_seed=rep, run_id=run_id)
    for shear_active, shear_passive, rep in to_do)
