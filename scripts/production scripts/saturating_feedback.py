...
import CE_simulation.saturating as sat
...
base_dir = "/data/Nikolas/GBE_simulation/runs/ltc_distribution/"

###############################################
# define local function for running simulation
###############################################

def run_phase_diagm_sim(orientation, T_plus, n_steps, random_seed=None, run_id=0):
    """
    ...
    save_dir = f"{base_dir}/{run_id}/orientation_{orientation}_TPlus_{T_plus}_randomSeed_{random_seed}"
    try:
    ...
    # for different feedbacks, we need to adjust the timescale to some baseline
    if T_plus < np.inf:
        dT_m = np.median([np.linalg.norm(sat.excitable_dt(np.array([he.rest for he in fc.hes]),
                                                          m=m, k_cutoff=k_cutoff))
                          for fc in mesh_initial.faces.values()])
        dT_sat = np.median([np.linalg.norm(sat.saturating_dt(np.array([he.rest for he in fc.hes]),
                                                             T_minus=T_minus, T_c=T_c, T_plus=T_plus))
                            for fc in mesh_initial.faces.values()])
        timescale = dT_m/dT_sat
    else:
        timescale = 1
    ...
    if T_plus < np.infty:
        def params_pattern(fid):
            if fid in passive_ids:
                return {"T_minus": 0, "T_c": 0, "T_plus": 0, "k": k/timescale, "is_active": False, "subtract_passive": False} 
            return {"T_minus": T_minus, "T_c": T_c, "T_plus": T_plus, "k": k/timescale,
                    "is_active": True, "subtract_passive": False}
    else:
        def params_pattern(fid):
            if fid in passive_ids:
                return {"m": 0, "k_cutoff": 0, "k": k, "is_active": False, "subtract_passive": False} 
    ...
    for i in range(n_steps-1):
        ...
        if T_plus < np.infty:
            mesh.euler_step(dt=dt*timescale, rhs_tension=sat.saturating_dt_act_pass, params=params_pattern,
                            rhs_rest_shape=rhs_rest_shape)
        else:
            mesh.euler_step(dt=dt*timescale, rhs_tension=tns.excitable_dt_act_pass_new_area, params=params_pattern,
                            rhs_rest_shape=rhs_rest_shape)
        ...
        if k_relax < np.infty:
            mesh.euler_step_relax(k_relax=k_relax/(dt*timescale), dt=dt*timescale)
        else:
...
T_pluses = [1.2, 3.33, np.inf]

reps = np.array([1, 2, 3]) # number of replicates # 1, 2, 3
to_do = list(itertools.product(orientations,T_pluses, reps))

n_steps = 600
run_id = "saturating_feedback_once_more"

###############################################
# run simulation
###############################################

Parallel(n_jobs=5, prefer=None)(delayed(run_phase_diagm_sim)( 
    orientation, T_plus,
    n_steps=n_steps, random_seed=rep, run_id=run_id)
    for orientation, T_plus, rep in to_do)
