...
base_dir = "/data/Nikolas/GBE_simulation/runs/bag_of_triangles/"

###############################################
# define local function for running simulation
###############################################

def run_phase_diagm_sim(orientation, isogonal, k_relax, k, n_steps, random_seed=None, run_id=0):
    """
    ...
    save_dir = f"{base_dir}/{run_id}/orientation_{orientation}_isogonal_{isogonal}_randomSeed_{random_seed}"
    try:
    ...
    if orientation == "parallel":
        mesh_initial, property_dict = drs.create_rect_initial(n_y, n_x, noise=noise_gaussian, initial_strain=initial_strain,
                                                              orientation='parallel', isogonal=isogonal,
        ...
        mesh_initial, property_dict = drs.create_rect_initial(n_x, n_y, noise=noise_gaussian, initial_strain=initial_strain,
                                                              orientation='orthogonal', isogonal=isogonal,
                                                              random_seed=random_seed,
                                                              boundaries=None, w_passive=0, w_passive_lr=0)
    ...
    for i in range(n_steps-1):
        ...
        if (time_module.time()-current_time) > timeout_iteration:
            ...
            break
        # set isogonal mode; now constant in time
        isogonal_current = isogonal
        for v in mesh.vertices.values():
            if v._vid in property_dict['passive_cells']:
                v.rest_shape = np.sqrt(3) * np.array([[1, 0],[0, 1]])
            else:
                v.rest_shape = np.sqrt(3) * np.array([[1-isogonal_current, 0],[0, 1+isogonal_current]])
...
isogonals = [0, 0.2, 0.333, 0.4,]
reps = np.array([1, 2, 3]) # number of replicates # 1, 2, 3
to_do = list(itertools.product(orientations, isogonals, [k_relax,], [k_passive,], reps))

n_steps = 501 # 500
run_id = "isogonal_mode"

###############################################
# run simulation
###############################################

Parallel(n_jobs=24, prefer=None)(delayed(run_phase_diagm_sim)( 
    orientation, isogonal, k_relax, k,
    n_steps=n_steps, random_seed=rep, run_id=run_id)
    for orientation, isogonal, k_relax, k, rep in to_do)
