...
base_dir = "/data/Nikolas/GBE_simulation/runs/for_revision/"

###############################################
# define local function for running simulation
###############################################

def run_phase_diagm_sim(orientation, angle_penalty, n_steps, random_seed=None, run_id=0):
    """
    ...
    save_dir = f"{base_dir}/{run_id}/orientation_{orientation}_anglePenalty_{angle_penalty}_randomSeed_{random_seed}"
    try:
    ...
    # package all arguments into a single dict to pass to the optimizer method
...
angle_penalties = [1000, 100, 10, 1]
reps = np.array([1, 2, 3]) # number of replicates

to_do = list(itertools.product(orientations, angle_penalties, reps))

n_steps = 501
run_id = "varying_epsilon"

###############################################
# run simulation
###############################################

Parallel(n_jobs=20, prefer=None)(delayed(run_phase_diagm_sim)( 
    orientation, angle_penalty, n_steps=n_steps, random_seed=rep, run_id=run_id)
    for orientation, angle_penalty, rep in to_do)
