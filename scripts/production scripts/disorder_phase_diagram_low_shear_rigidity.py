...
def run_phase_diagm_sim(initial_strain, eta, mod_shear, n_steps, random_seed=None, run_id=0):
    """
    ...
    save_dir = f"{base_dir}/{run_id}/eta_{eta}_anisotropy_{initial_strain}_modShear_{mod_shear}_randomSeed_{random_seed}"
...
initial_strains = np.array([0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45])   
etas = [0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9] #
mod_shears = [0.5, 0.25, 0]
reps = [1, 2, 3]
to_do = list(itertools.product(initial_strains, etas, mod_shears, reps))

n_steps = 500
run_id = "phase_diag_low_shear_rigidity"

###############################################
# run simulation
###############################################

Parallel(n_jobs=40, prefer=None)(delayed(run_phase_diagm_sim)( 
    initial_strain, eta, mod_shear,
    n_steps=n_steps, random_seed=rep, run_id=run_id)
    for initial_strain, eta, mod_shear, rep in to_do)
